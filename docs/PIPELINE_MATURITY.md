...
| Diffusion video decoder | `generate --video-decoder diffusion` (LTX 2.5 packs only) | Opt-in alternative to the default conv VAE decoder; sharper output, several times slower. Single tile in v1 — `LTX2_DIFFVAE_MAX_TOKENS` refuses oversized decodes, and current stage-5 neighborhood attention hits a separate Metal buffer-count ceiling above roughly 512×768×25 on 32 GB Macs (tiling lands in a follow-up). Reproduces upstream's default `chunked_eager` mode, including its edge-replicated borders. |
